    ...
    if (image_src.length) {
      return (
        <Link className={styles['feed__item-title-link']} to={post_src}>
          <img src={`https://images.weserv.nl/?url=${image_src[0]}&w=960&output=webp&we`} style={{'width':'100%'}} alt="Main article image" />
        </Link>
        )
    }
  }
      ...
      <div className={styles['feed__item']} key={edge.node.fields.slug}>
        ...
        <div className={styles['feed__item-content']}>
          ...
          <p className={styles['feed__item-p']}>
            By <a href={`https://smoke.io/@${edge.node.fields.author}`} style={{ color: 'green' }}>@{edge.node.fields.author}</a><br/>
